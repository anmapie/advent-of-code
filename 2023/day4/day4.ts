...
function getScratchCard(line: string): ScratchCard {
  const numbersString = line.split(": ")[1];
  const numberSets = numbersString.split(" | ");

  return {
    winningNumbers: stringToNumberArr(numberSets[0]),
    myNumbers: stringToNumberArr(numberSets[1]),
    instances: 1,
  };
}

function getWinningNumberCount(card: ScratchCard): number {
  return card.myNumbers.reduce(
    (winnerCount, currNumber) =>
      (winnerCount += card.winningNumbers.includes(currNumber) ? 1 : 0),
    0
  );
}

// winning numbers are worth 1 pt for the first match, double the current
// total for each subsequent match
...
function calculateTotalCardInstances(scratchCards: ScratchCard[]): number {
  scratchCards.forEach((card, index) => {
    const winningNumberCount = getWinningNumberCount(card);

    // winning cards win a copy of N subsequent cards, where N is the count
    // of winning numbers
    for (let i = 1; i <= winningNumberCount; i++) {
      if (scratchCards[index + i] != null) {
        scratchCards[index + i].instances += card.instances;
      }
    }
  });

  return scratchCards.reduce(
    (totalCards, card) => (totalCards += card.instances),
    0
  );
}

const lines = readFileIntoArray();
const scratchCards: ScratchCard[] = lines.map((line) => getScratchCard(line));

// part 1: calculate all points
let cardPointsSum = 0;
for (const card of scratchCards) {
  cardPointsSum += calculateCardPoints(card);
}

// part 2: calculate all card instances
const cardInstancesSum = calculateTotalCardInstances(scratchCards);

console.log(`Total points: ${cardPointsSum}`);
console.log(`Total cards: ${cardInstancesSum}`);
