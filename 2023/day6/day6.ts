...
// line format
// 'Time:      7  15   30'
function getNumbersFromLine(line: string): number[] {
  return stringToNumberArr(line.split(":")[1]);
}

// fancy fancy:
// max possible race distance occurs when button press time === race time / 2
// on either side of that max, there will be an equal number of possible ways
// to win; so we can calculate the max + the number of possibilities in one
// direction to speed this up
function getPossibleRaceWinsCount(
  raceTime: number,
  recordDistance: number
): number {
  let buttonPressTime = Math.round(raceTime / 2);
  let currDistance = (raceTime - buttonPressTime) * buttonPressTime;
  let winCount = 0;

  while (currDistance > recordDistance) {
    winCount++;
    buttonPressTime++;
    currDistance = (raceTime - buttonPressTime) * buttonPressTime;
  }

  // if we had an even race time, we'll have counted an extra win
  return winCount * 2 - (raceTime % 2 === 0 ? 1 : 0);
}

const lines = readFileIntoArray();
const times = getNumbersFromLine(lines[0]);
const distances = getNumbersFromLine(lines[1]);

const winCounts = [];
times.forEach((time, index) =>
  winCounts.push(getPossibleRaceWinsCount(time, distances[index]))
);

console.log(`Win counts: ${winCounts}`);
console.log(`Product of wins: ${computeProductOfArray(winCounts)}`);
